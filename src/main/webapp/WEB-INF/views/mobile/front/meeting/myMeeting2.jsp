...
<body>
	...
	<div style="background-color: #FFFFFF; height: 40px;margin: 10px">
			...
			<div style="background-color:#4AACC5; position:relative; top:37px;left:38%; width: 96px;height: 4px;">
			</div>
							...
							<ul id="item1" class="mui-table-view">
								<c:forEach items="${list }" var="item">
									...
									<div style="float: left;width: 120px">
										<img src="${item.banner }" width="120px" height="90px">
									</div>
									<div style="float: left;margin-left: 10px">
										<h4 style="color: #333333">${fns:abbr(item.subject,23)}</h3>
										<fmt:formatDate value="${item.startTime }" pattern="yy-M-d H:m" />&nbsp;~&nbsp;<fmt:formatDate value="${item.endTime }" pattern="yy-M-d H:m" /><br>${item.address }
									</div></li>
								</c:forEach>
							</ul>
						</div>
