...
<body>
	<header class="mui-bar mui-bar-nav" style="background-color: #4AACC5;">
		...
		<h1 class="mui-title" style="font-size: 16px; color: white; font-weight: normal">消息列表</h1>
	</header>
						...
						<div class="mui-scroll">
							<ul id="item1" class="mui-table-view" style="background-color: transparent;">
								<c:forEach items="${list }" var="item">
									<li>
										<span><h5 style="float:left; width: 80%">${item.subject}</h5><p style="float:right;text-align:right; width:20%;"><fmt:formatDate value="${item.updateDate }" pattern="M-d H:m" /></p></span>
										<span><h4 >${item.content}</h4></span>
										</li>
								</c:forEach>
							</ul>
						</div>
