...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/stats/article.do">信息量统计</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/stats/article.do" method="post" class="breadcrumb form-search">
		<div>
			<label>归属栏目：</label><sys:treeselect id="category" name="categoryId" value="${paramMap.id}" labelName="categoryName" labelValue="${paramMap.name}"
				title="栏目" url="/cms/category/treeData.do" module="article" cssClass="input-small" allowClear="true"/>
			<label>归属机构：</label><sys:treeselect id="office" name="officeId" value="${paramMap.office.id}" labelName="officeName" labelValue="${paramMap.office.name}" 
				title="机构" url="/sys/office/treeData.do" cssClass="input-small" allowClear="true"/>
