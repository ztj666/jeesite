...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/cms/article.do?category.id=${article.category.id}">文章列表</a></li>
		
		<li class="active">
		    <a href="<c:url value='${fns:getAdminPath()}/cms/article/form.do?id=${article.id}&category.id=${article.category.id}'>
		       <c:param name='category.name' value='${article.category.name}'/></c:url>">文章
		       <shiro:hasPermission name="cms:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission>
		       <shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission>
		    </a>
		</li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/save.do" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span class="help-inline">多个关键字，用逗号分隔。</span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<script type="text/javascript">
					...
					$.getJSON("${ctx}/cms/article/findByIds.do",{ids:$("#articleDataRelation").val()},function(data){
					...
					$("#relationButton").click(function(){
						top.$.jBox.open("iframe:${ctx}/cms/article/selectList.do?pageSize=8", "添加相关",$(top.document).width()-220,$(top.document).height()-180,{
		...
		<c:if test="${not empty article.id}">
			<div class="control-group">
				...
				<div class="controls">
					<input id="btnComment" class="btn" type="button" value="查看评论" onclick="viewComment('${ctx}/cms/comment.do?module=article&contentId=${article.id}&status=0')"/>
					<script type="text/javascript">
