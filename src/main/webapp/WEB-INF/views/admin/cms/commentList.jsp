...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/comment.do">评论列表</a></li><%--
		<shiro:hasPermission name="cms:comment:edit"><li><a href="${ctx}/cms/comment/form.do?id=${comment.id}">评论添加</a></li></shiro:hasPermission> --%>
	</ul>
	<form:form id="searchForm" modelAttribute="comment" action="${ctx}/cms/comment.do" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="comment">
			<tr>
				...
				<td><shiro:hasPermission name="cms:comment:edit">
					<c:if test="${comment.delFlag ne '2'}"><a href="${ctx}/cms/comment/delete.do?id=${comment.id}${comment.delFlag ne 0?'&isRe=true':''}" 
						onclick="return confirmx('确认要${comment.delFlag ne 0?'恢复审核':'删除'}该审核吗？', this.href)">${comment.delFlag ne 0?'恢复审核':'删除'}</a></c:if>
					<c:if test="${comment.delFlag eq '2'}"><a href="${ctx}/cms/comment/save.do?id=${comment.id}">审核通过</a></c:if></shiro:hasPermission>
				</td>
			</tr>
