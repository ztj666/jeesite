...
<head>
	...
	<script type="text/javascript">
    	...
    	function updateSort() {
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/category/updateSort.do");
	    	$("#listForm").submit();
    	}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/category.do">栏目列表</a></li>
		<shiro:hasPermission name="cms:category:edit"><li><a href="${ctx}/cms/category/form.do">栏目添加</a></li></shiro:hasPermission>
	</ul>
		...
		<table id="treeTable" class="table table-striped table-bordered table-condensed">
			...
			<c:forEach items="${list}" var="tpl">
				<tr id="${tpl.id}" pId="${tpl.parent.id ne '1'?tpl.parent.id:'0'}">
					<td><a href="${ctx}/cms/category/form.do?id=${tpl.id}">${tpl.name}</a></td>
					...
					<td>
						...
						<shiro:hasPermission name="cms:category:edit">
							<a href="${ctx}/cms/category/form.do?id=${tpl.id}">修改</a>
							<a href="${ctx}/cms/category/delete.do?id=${tpl.id}" onclick="return confirmx('要删除该栏目及所有子栏目项吗？', this.href)">删除</a>
							<a href="${ctx}/cms/category/form.do?parent.id=${tpl.id}">添加下级栏目</a>
						</shiro:hasPermission>
					</td>
				</tr>
			</c:forEach>
