...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/link.do?category.id=${link.category.id}">链接列表</a></li>
		<shiro:hasPermission name="cms:link:edit"><li><a href="<c:url value='${fns:getAdminPath()}/cms/link/form.do?id=${link.id}&category.id=${link.category.id}'><c:param name='category.name' value='${link.category.name}'/></c:url>">链接添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="link" action="${ctx}/cms/link.do" method="post" class="breadcrumb form-search">
		...
		<label>栏目：</label><sys:treeselect id="category" name="category.id" value="${link.category.id}" labelName="category.name" labelValue="${link.category.name}"
					title="栏目" url="/cms/category/treeData.do" module="link" notAllowSelectRoot="false" cssClass="input-small"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="link">
			<tr>
				...
				<td><a href="${ctx}/cms/link/form.do?id=${link.id}" title="${link.title}">${fns:abbr(link.title,40)}</a></td>
				<td>${link.weight}</td>
				<td>${link.user.name}</td>
				<td><fmt:formatDate value="${link.updateDate}" type="both"/></td>
				<shiro:hasPermission name="cms:link:edit"><td>
    				<a href="${ctx}/cms/link/form.do?id=${link.id}">修改</a>
					<a href="${ctx}/cms/link/delete.do?id=${link.id}${link.delFlag ne 0?'&isRe=true':''}&categoryId=${link.category.id}" onclick="return confirmx('确认要${link.delFlag ne 0?'发布':'删除'}该链接吗？', this.href)" >${link.delFlag ne 0?'发布':'删除'}</a>
